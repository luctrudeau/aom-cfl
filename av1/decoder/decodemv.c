...
static int read_cfl_alphas(FRAME_CONTEXT *const ec_ctx, aom_reader *r,
  ...
  int js =
      aom_read_symbol(r, ec_ctx->cfl_sign_cdf, CFL_JOINT_SIGNS, "cfl:sign");
  int ind = 0;
  ...
  if (sign_u != CFL_SIGN_ZERO)
    ind += aom_read_symbol(r, ec_ctx->cfl_alpha_cdf[js][CFL_PRED_U], 16,
                           "cfl:alpha")
           << 4;
  if (sign_v != CFL_SIGN_ZERO)
    ind += aom_read_symbol(r, ec_ctx->cfl_alpha_cdf[js][CFL_PRED_V], 16,
                           "cfl:alpha");
